...
\begin{itemize}
 \justifying
 \item To develop a Finite Element code for the Vorticity-Streamfunction Formulation 
       with species transport equation using the Arbitrary Lagrangian-Eulerian (ALE) approach and semi-Lagrangian Method \\
...
\begin{tikzpicture}[scale=0.8,square/.style={regular polygon,regular polygon sides=4,inner sep=0}]
 ...
 % Eulerian legend 
 \draw [latex-latex] (-0.3,2.5) -- (-0.3,3.5);
 \node at (-0.5,3) {t};
...
\begin{equation*}
\mathbf{\hat{x}_{i}} = \sum_{i}^{np} \sum_{j}^{N_{1}} w_{ij} (\mathbf{x}_{j} - \mathbf{x}_{i})
\end{equation*}
